...
import com.myropcb.nest.data.Segment;
import com.myropcb.nest.util.coor.ClipperCoor;
import com.myropcb.nest.util.coor.NestCoor;
...
public class CommonUtil {
    ...
    public static ClipperCoor toClipperCoor(double x , double y ){
        return new ClipperCoor((long)(x* NestConfig.CLIIPER_SCALE) , (long) (y * NestConfig.CLIIPER_SCALE));
    }
    ...
    public static NestCoor toNestCoor(long x , long y ){
        return new NestCoor(((double)x/ NestConfig.CLIIPER_SCALE) , ((double)y/ NestConfig.CLIIPER_SCALE));
    }
    ...
    public static List<NestPath> polygonOffset(NestPath polygon , double offset){
        ...
        int miterLimit = 2;
        ClipperOffset co = new ClipperOffset(miterLimit , polygon.config.CURVE_TOLERANCE * NestConfig.CLIIPER_SCALE);
        co.addPath(p, Clipper.JoinType.ROUND , Clipper.EndType.CLOSED_POLYGON);

        Paths newpaths = new Paths();
        co.execute(newpaths , offset * NestConfig.CLIIPER_SCALE);
