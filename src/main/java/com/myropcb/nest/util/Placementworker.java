package com.myropcb.nest.util;

import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.myropcb.nest.data.*;
import com.myropcb.nest.util.coor.ClipperCoor;
...
public class Placementworker {
     ...
     */
    public Placementworker(NestPath binPolygon, NestConfig config, Map<String, List<NestPath>> nfpCache) {
    ...
    public Result placePaths(List<NestPath> paths){
        ...
        while(paths.size() > 0 ){
            ...
            for(int i = 0; i <paths.size() ; i ++){
                ...
                for(int j = 0 ; j<placed.size(); j ++){
                    ...
                    for(int k = 0 ; k <nfp.size();k++){
                        ...
                        for(int m = 0 ; m< clone.size() ; m++){
                            ...
                            clone.get(m).setX(clx + (long)(placements.get(j).x * NestConfig.CLIIPER_SCALE));
                            clone.get(m).setY(cly + (long)(placements.get(j).y * NestConfig.CLIIPER_SCALE));
                        }
                        clone = clone.cleanPolygon(0.0001 * NestConfig.CLIIPER_SCALE);
                        double area = Math.abs(clone.area());
                        if( clone.size() > 2 && area > 0.1 * NestConfig.CLIIPER_SCALE * NestConfig.CLIIPER_SCALE){
                ...
                finalNfp = finalNfp.cleanPolygons(0.0001 * NestConfig.CLIIPER_SCALE);
                for(int j  =0 ; j< finalNfp.size() ; j ++){
                    double area = Math.abs(finalNfp.get(j).area());
                    if( finalNfp.get(j).size() < 3 || area < 0.1 * NestConfig.CLIIPER_SCALE * NestConfig.CLIIPER_SCALE){
                        finalNfp.remove(j);
                        j--;
                    }
                }
    ...
    public static NestPath toNestCoordinates(Path polygon){
        ...
        for(int i = 0 ; i< polygon.size() ; i ++){
            Segment s = new Segment((double)polygon.get(i).getX()/ NestConfig.CLIIPER_SCALE , (double)polygon.get(i).getY()/ NestConfig.CLIIPER_SCALE);
            clone.add(s);
        }
        return clone ;
    }
