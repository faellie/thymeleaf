...
import com.myropcb.nest.algorithm.GeneticAlgorithm;
import com.myropcb.nest.algorithm.Individual;
import com.myropcb.nest.data.*;
import com.myropcb.nest.data.Vector;
import com.myropcb.nest.util.*;
...
public class Nest {
     ...
     * @param count     迭代计算次数
     */
    public Nest(NestPath binPath, List<NestPath> parts, NestConfig config, int count) {
     ...
     * @return
     */
    public Result launchWorkers(List<NestPath> tree , NestPath binPolygon , NestConfig config ){
        launchcount++;
        if(GA == null ){
...
//        }
    ...
    public void setConfig(NestConfig config) {
        this.config = config;
    }
